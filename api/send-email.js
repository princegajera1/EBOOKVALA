export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({ error: "Resend API Key is missing on the server config." });
    }

    // Default sender address for Resend sandbox/onboarding.
    // If you verify your domain (e.g. ebookvala.com), update this to 'hello@ebookvala.com' or 'noreply@ebookvala.com'
    ...
    if (!response.ok) {
      return res.status(response.status).json({ error: result.message || "Failed to trigger Resend API email request." });
    }
