...
export const StaggeredMenu = ({
  position = 'right',
  colors = ['#111111', '#1C1C1E'], // Dark luxury ink theme colors
  items = [],
  socialItems = [],
  displaySocials = true,
  ...
  // Handle ESC key to close StaggeredMenu
  React.useEffect(() => {
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#111111', '#1C1C1E'];
          let arr = [...raw];
          if (arr.length >= 3) {
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="sm-logo" aria-label="Logo">
          <span className="font-display font-black tracking-tight select-none text-[19px]">
            <span className={open ? "text-white" : "text-zinc-950 dark:text-white"}>EBOOK</span>
