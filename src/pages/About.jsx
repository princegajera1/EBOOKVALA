        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-10 border-y border-brand-border py-16 mb-20 md:mb-28">
          {stats.map((stat, idx) => (
            <div key={idx}>
              ...
              <p className="text-xs text-brand-text-secondary font-bold mt-1 uppercase tracking-wider">{stat.label}</p>
            </div>
          ))}
        </div>
            ...
            {values.map((val, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-col gap-3">
                  <div className="h-10 w-10 rounded-full bg-brand-bg-secondary border border-brand-border flex items-center justify-center text-brand-text shrink-0">
