...
import { Sparkles, Trophy, BookOpen, Clock, Layers, Flame, Play, Star, Calendar } from "lucide-react";
...
export const Overview = ({ user, books = [] }) => {
  ...
  const streak = user?.streak || 5; // Fallback mock reading streak
  const streakGoal = 10;
  ...
  return (
    <div className="flex flex-col gap-6 text-left select-none font-sans transition-colors duration-300">
      ...
      {/* 1. COMPACT HERO SECTION (Max height: 180px) */}
      ...
      {/* 2. MAIN LAYOUT GRID (Continue Reading + Stats) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        
        {/* Continue Reading Card (Left 2 columns) */}
        <div className="lg:col-span-2 bg-brand-card border border-brand-border/70 rounded-[24px] p-6 shadow-brand flex flex-col justify-between gap-5 relative overflow-hidden">
          ...
          {activeBook ? (
              ...
              <div className="flex flex-col justify-between flex-grow text-center sm:text-left">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-auto">
                  ...
                  <Button 
                    ...
                    className="rounded-full text-[11px] font-bold h-9 px-4.5 bg-brand-accent hover:scale-[1.02] shadow-sm flex items-center gap-1.5 select-none shrink-0"
                  >
            ...
            <div className="flex flex-col items-center justify-center py-6 text-center">
              ...
              <Button onClick={() => navigate("/marketplace")} variant="outline" className="mt-4 rounded-full text-[10px] font-bold h-8.5 px-4">
                Explore Marketplace
              </Button>
            </div>
          )}
        ...
        {/* Dynamic Glass Stats Card (Right 1 column) */}
      ...
      <div className="bg-brand-card border border-brand-border/70 rounded-[24px] p-6 shadow-brand text-left">
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-7 sm:grid-cols-12 md:grid-cols-28 lg:grid-cols-30 gap-1.5 overflow-x-auto py-2 pr-1 no-scrollbar">
            {Array.from({ length: 30 }).map((_, i) => {
              // Mock consistency levels: 0 (none) to 3 (very high)
              const level = i % 7 === 0 ? 0 : i % 5 === 0 ? 3 : i % 3 === 0 ? 2 : 1;
              const levelColors = [
              ...
              return (
                <div 
                  key={i} 
                  title={`Day ${i + 1}: ${level === 0 ? 'No reading' : level === 1 ? 'Read 15m' : level === 2 ? 'Read 45m' : 'Read 1.5h+'}`}
                  className={`h-5 w-5 rounded-[4px] shrink-0 cursor-pointer hover:scale-105 duration-200 transition-transform ${levelColors[level]}`}
                />
              );
            })}
          </div>
          <div className="flex justify-end items-center gap-2 text-[8px] font-mono font-bold text-brand-text-secondary uppercase tracking-widest">
            ...
            <div className="h-3.5 w-3.5 rounded bg-brand-accent/15" />
            <div className="h-3.5 w-3.5 rounded bg-brand-accent/35" />
            <div className="h-3.5 w-3.5 rounded bg-brand-accent/65" />
            <span>More</span>
          </div>
        </div>
      </div>
